...
echo "  Frontend:     $(grep 'frontend:' ansible/group_vars/all.yml | awk '{print $2}')"
echo "  Eureka:       $(grep 'eureka:' ansible/group_vars/all.yml | grep -A 10 'replicas:' | head -1 | awk '{print $2}')"
echo "  Gateway:      $(grep 'gateway:' ansible/group_vars/all.yml | grep -A 10 'replicas:' | head -2 | tail -1 | awk '{print $2}')"
echo "  User Service: $(grep 'user:' ansible/group_vars/all.yml | grep -A 10 'replicas:' | head -3 | tail -1 | awk '{print $2}')"
echo "  Travel:       $(grep 'travel:' ansible/group_vars/all.yml | grep -A 10 'replicas:' | head -4 | tail -1 | awk '{print $2}')"
echo "  Payment:      $(grep 'payment:' ansible/group_vars/all.yml | grep -A 10 'replicas:' | head -5 | tail -1 | awk '{print $2}')"
echo ""
echo "To deploy with custom replicas, run:"
echo "  ansible-playbook ansible/playbooks/deploy_services.yml -e '{\"microservices_replicas\":{\"user\":5,\"travel\":5,\"payment\":5}}'"
